<?php
   ...
   if(isset($_GET['id']) AND $_GET['id']>0)
   {
     ...
     $reponse=$base->query("SELECT id, nom, prenom, adresse, numtelephone, rolee FROM patients WHERE rolee='femmeEnceinte' ORDER BY id DESC");
            
    if(isset($_GET['recherche']) AND !empty($_GET['recherche'])){
        $recherche=htmlspecialchars($_GET['recherche']);
        $reponse= $base->query('SELECT id, nom, prenom, adresse, numtelephone, rolee FROM patients WHERE rolee="femmeEnceinte" AND  nom LIKE "%'.$recherche.'%" ORDER BY id DESC' );
        if($reponse->rowCount() == 0) {
            $reponse= $base->query('SELECT id, nom, prenom,adresse, numtelephone, rolee FROM patients WHERE rolee="femmeEnceinte" AND prenom LIKE "%'.$recherche.'%" ORDER BY id DESC');
        }
        if($reponse->rowCount() == 0) {
            $reponse= $base->query('SELECT id, nom, prenom, adresse, numtelephone, rolee FROM patients WHERE rolee="femmeEnceinte" AND numtelephone LIKE "%'.$recherche.'%" ORDER BY id DESC');
        }
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Liste des femmes enceintes</h4>
                                    ...
                                    if($reponse->rowCount()>0){
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td class="fs-1">
                                                    <a href="infofemme.php?patient=<?= $donnees['id']?>" class="text-success"><i class="fa fa-eye" style="font-size: 30px; color:#003399;"></i></a>
                                                </td>
                                                <td class="fs-1">
                                                    <a href="carnetf.php?edit=<?= $donnees['id']?>" class=""><i class="fa fa-pencil-square-o" style="font-size: 30px; color:#22780F;"></i></a>
                                                </td>
                                            </tr>
                                        </tbody>
                                            <?php
